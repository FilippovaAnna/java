...
public class Main {
    public static void main(String[] args) {
        ...
        cat1.setSwimmingSpeed(1);
        cat1.setRunningSpeed(15);
        cat1.setJumpingSpeed(10);

        Cat cat2 = new Cat("Морис", "рыжий", 2);
        cat2.setSwimmingSpeed(1);
        cat2.setRunningSpeed(20);
        cat2.setJumpingSpeed(10);

        Wolf wolf = new Wolf("Тревор", "серый", 3);
        wolf.setSwimmingSpeed(10);
        wolf.setRunningSpeed(25);
        wolf.setJumpingSpeed(5);

        Turtle turtle = new Turtle("Тортила", "зеленый", 4);
        turtle.setSwimmingSpeed(10);
        turtle.setRunningSpeed(5);
        turtle.setJumpingSpeed(1);

        Team team = new Team("Первая команда");
        team.addToTeam(cat1);
        team.addToTeam(cat2);
        team.addToTeam(wolf);
        team.addToTeam(turtle);
        team.showTeam();

        Course[] courses = {new Pool(15), new Stadium(20), new Pool(35)};

        for (Course course : courses) {
            System.out.println(course);
            course.getTimeToOvercome(cat1);
            course.getTimeToOvercome(cat2);
            course.getTimeToOvercome(wolf);
            course.getTimeToOvercome(turtle);
        }
